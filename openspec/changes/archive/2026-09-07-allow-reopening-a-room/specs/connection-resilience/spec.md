...
**Reason**: Replaced by "A link that is not a game is not retried". The rule that the page stops
rather than retrying something unrecoverable is kept exactly; what changed is which case that is.
A room that has gone is no longer unrecoverable — the next attempt creates one — so the only thing
left to stop for is an identifier the server refuses.

Its two other scenarios go with it because both describe behaviour that has been deliberately
removed: an expired invitation no longer produces an explanation, it produces the room, and a
restarted server no longer ends anybody's game.
